...
public class NineGridlayout extends ViewGroup {
    ...
    public NineGridlayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        gap = dip2px(this.context, ITEM_GAP);
        defaultWidth = defaultHeight = dip2px(this.context, DEFAULT_WIDTH);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (adapter != null && adapter.getCount() > 0) {
            ...
            if (childrenCount == 1) {
                ...
                singleWidth = (totalWidth - gap * (adapter.getRuler() - 1)) / adapter.getRuler();
    ...
    private void generateChildrenLayout() {
        rows=(adapter.getCount()%adapter.getRuler())==0 ? adapter.getCount()/adapter.getRuler() : (adapter.getCount()/adapter.getRuler()+1);
        columns=adapter.getRuler();
    }
